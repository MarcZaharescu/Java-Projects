...
import gameEngine.flags.Poison;
...
/**
 * PoisonDart Weapon Class
 * 
 *
 */
public class PoisonDart implements Weapon {
	...
	public PoisonDart() {
		readImg("img" + File.separator + "weapSprites" + File.separator+ "poisondart.png");
		readPanelImg("img" + File.separator + "weapPanel" + File.separator+ "poisondartpanel.png");
		description = "Fires a parabola missle that poisons the players on the ground it lands on.";
		details = "PoisonDart - Fires a parabola poison missle that poisons the players within 200px distance.\n"
				+ "Poison deals 100 hp per turn and last for 3 turns.\n"
				+ "Players that get hit by another poison will be refreshed.\n\n";
	}
	...
	public void animateShot(BufferedImage fancyMap,
		...
		updateAfterShot(path.get(path.size() - 1), engine.getBitMap(), characters,
				explosiveRange, active, engine);
	}

	/**
	 * Poison the affected players
	 * 
	 * @param center
	...
	public void updateAfterShot(Coordinates center, Map bitMap,
		...
		// if character is affected, add poison flag to it
		...
		for (Character affectedCharacter : affectedCharacters) {
			if (affectedCharacter.getTeamNumber() != teamNumber) {
				affectedCharacter.addFlag(new Poison(3, affectedCharacter));
			}
	...
	/**
	 * No explosion for the poison dart.
	 */
